  ...
  const handleClick = async () => {
    if (thumbnailRef.current) {
      ...
      if (positionData) {
        ...
        sessionStorage.setItem('thumbnailState', JSON.stringify(transitionState));
...
export const ItemDetailPage = ({ itemId, imageUrl, title, onBack, children }) => {
  ...
  React.useEffect(() => {
    ...
    // 2순위: 세션 스토리지에서 복원
      ...
      // 3순위: 로컬 스토리지에서 스크롤만 복원 (백업)
        ...
        const localScrollData = localStorage.getItem('lastScrollPosition');
        if (localScrollData) {
          try {
  ...
  const handleAnimationComplete = () => {
    ...
    // sessionStorage.removeItem('thumbnailState'); // 삭제하지 않음
    // localStorage.removeItem('lastScrollPosition'); // 삭제하지 않음
