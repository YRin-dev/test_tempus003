...
const MotionBlurFilter = () => (
  <svg style={{ position: 'absolute', width: 0, height: 0 }}>
    <defs>
      <filter
        id="vertical-motion-blur-filter"
        x="-50%"
        y="-50%"
        width="200%"
        height="200%"
...
/**
 * 가로 스크롤 중에 세로 스크롤 하이재킹을 구현하는 컴포넌트
 * children을 세로로 배치하고, 스크롤 시 세로 방향으로 이동하도록 구현
 * Lenis와 GSAP ScrollTrigger를 함께 사용
 */
function VerticalScrollSection({
  children,
  width = '100vw',
  backgroundColor,
  sectionTitle,
  ...
  itemHeight = '60%', // 아이템 높이 (기본값 60% 화면 높이)
  visibleItems = 1.5, // 한 화면에 보이는 아이템 수 (1.5는 한 개 완전히 보이고 다음 아이템이 절반 보임)
  sidePaddingRatio = 0.05, // 위아래 여백 비율 (기본값: 화면 높이의 5%)
  ...
  const initScrollTrigger = useCallback(() => {
    ...
    scrollTriggers.forEach((trigger) => {
      if (trigger.vars.id === `vertical-scroll-${sectionRef.current?.id}`) {
        trigger.kill();
      }
    });
    ...
    console.log('🔄 Initializing VerticalScrollSection with viewport:', {
      windowHeight: window.innerHeight,
      containerHeight: scrollContainer?.clientHeight || 'N/A',
      screenSize:
    ...
    // 블러 애니메이션 요소 참조 저장
    blurAnimationRef.current = document.getElementById(
      'vertical-blur-animation'
    );
    ...
    // 뷰포트 높이 계산 (스크롤 컨테이너가 있으면 해당 높이, 없으면 window 높이)
    const viewportHeight = scrollContainer
      ? scrollContainer.clientHeight
      : window.innerHeight;

    // 반응형 아이템 높이 계산
    let responsiveItemHeight = itemHeight;

    // 화면 크기별 아이템 높이 자동 조정
    if (typeof itemHeight === 'string' && itemHeight.includes('%')) {
      // 퍼센트로 지정된 경우 화면 크기에 따라 조정
      const basePercentage = parseFloat(itemHeight);
      if (viewportHeight < 768) {
        // 모바일
        responsiveItemHeight = Math.min(basePercentage + 20, 95) + '%'; // 모바일에서는 더 높게
      } else if (viewportHeight < 1024) {
        // 태블릿
        responsiveItemHeight = Math.min(basePercentage + 10, 80) + '%'; // 태블릿에서는 약간 높게
      }
    }

    // 아이템 높이 계산 (퍼센트 -> 실제 픽셀)
    const itemHeightPixel =
      typeof responsiveItemHeight === 'string' &&
      responsiveItemHeight.includes('%')
        ? (viewportHeight * parseFloat(responsiveItemHeight)) / 100
        : typeof responsiveItemHeight === 'number'
        ? responsiveItemHeight
        : viewportHeight * 0.6; // 기본값은 60%

    // 반응형 간격 조정
    const responsiveGap = viewportHeight < 768 ? Math.max(gap * 0.5, 10) : gap; // 모바일에서는 간격 줄이기

    // 반응형 여백 조정
    const responsiveSidePaddingRatio =
      viewportHeight < 768 ? sidePaddingRatio * 0.5 : sidePaddingRatio; // 모바일에서는 여백 줄이기
    const bottomPadding = viewportHeight * responsiveSidePaddingRatio;

    // 컨테이너 전체 높이 계산
    // 첫 번째 아이템부터 마지막 아이템까지 + 아이템 간 gap + 아래쪽 여백
    const containerHeight =
      totalItems * itemHeightPixel +
      (totalItems - 1) * responsiveGap +
      bottomPadding;

    // 스크롤 종료 위치 계산 (마지막 아이템이 완전히 보이도록)
    // 마지막 아이템이 화면 위쪽 가장자리에 올 때까지 스크롤
    const maxScroll = containerHeight - itemHeightPixel;

    // 컨테이너 높이 설정
    gsap.set(container, { height: containerHeight });

    // 각 자식 요소에 스타일 적용 (높이와 마진만 설정, 다른 스타일 속성은 변경하지 않음)
    container
      .querySelectorAll('.vertical-section-item')
      .forEach((item, index) => {
        gsap.set(item, {
          height: itemHeightPixel,
          marginTop: index === 0 ? 0 : responsiveGap, // 첫 번째 아이템은 위쪽 가장자리에 붙임
          marginBottom: index === totalItems - 1 ? bottomPadding : 0, // 마지막 아이템에만 아래쪽 여백
        });
      });

    // ScrollTrigger 설정
    const scrollTrigger = ScrollTrigger.create({
      id: `vertical-scroll-${section.id}`,
      trigger: section,
      ...
      onUpdate: (self) => {
        ...
        if (enableMotionBlur && blurAnimationRef.current) {
          ...
          if (Math.abs(velocity) > velocityThreshold) {
            ...
            // 애니메이션 요소 업데이트
            blurAnimationRef.current.setAttribute(
              'values',
            ...
            // 모든 이미지에 필터 적용
            container
              .querySelectorAll('.vertical-section-item img')
              .forEach((img) => {
                img.style.filter = 'url(#vertical-motion-blur-filter)';
              });
          } else {
            // 속도가 낮을 때는 블러 제거
      ...
      onRefresh: () => {
        ...
        console.log('VerticalScrollSection refreshed', {
          totalItems,
          itemHeightPixel,
          bottomPadding,
          gap: responsiveGap,
          ...
          scrollContainer: scrollContainer
            ? scrollContainer.className || 'detected-container'
            : 'window',
        });
      },
    });
    ...
    resizeObserverRef.current = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        if (widthDiff > 20 || heightDiff > 20) {
          ...
          if (!isScrolling) {
            // 스크롤 중이 아닐 때만 재초기화
            clearTimeout(window.verticalScrollComponentResizeTimer);
            window.verticalScrollComponentResizeTimer = setTimeout(() => {
            ...
            console.log(
              '🔄 Resize detected but skipping due to active scrolling'
            );
          }
        }
      }
    ...
    const handleWindowResize = () => {
      ...
      clearTimeout(window.verticalScrollWindowResizeTimer);
      window.verticalScrollWindowResizeTimer = setTimeout(() => {
  ...
  useEffect(() => {
    ...
    if (sectionRef.current && !sectionRef.current.id) {
      sectionRef.current.id = `vertical-section-${Math.random()
        .toString(36)
        .substr(2, 9)}`;
    }
    ...
    >
      ...
      >
        ...
        {React.Children.map(children, (child, index) => (
          <Box
            key={index}
            className="vertical-section-item"
            sx={(theme) => ({
              width: '100%',
              display: 'flex',
              justifyContent: 'center',
              alignItems: 'center',
              padding: {
                xs: `${Math.max(gap * 0.5, 5)}px 0`, // 모바일에서는 패딩 줄이기
                md: `${gap}px 0`,
              },
...
VerticalScrollSection.propTypes = {
