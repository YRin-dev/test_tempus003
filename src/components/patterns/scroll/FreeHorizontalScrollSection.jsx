...
function FreeHorizontalScrollSection({ 
  ...
  useEffect(() => {
    ...
    console.log('🚀 [FreeHorizontal] Initializing with GSAP context');
    ...
    contextRef.current = gsap.context(() => {
      ...
      gsap.delayedCall(0.1, () => {
        ...
        ScrollTrigger.create({
          ...
          onRefresh: () => {
            console.log('🔄 FreeHorizontalScrollSection refreshed with context');
          }
        });
      });
    ...
    const handleWindowResize = () => {
      console.log('🔄 [FreeHorizontal] Window resize detected');
      
      clearTimeout(window.freeHorizontalResizeTimer);
      window.freeHorizontalResizeTimer = setTimeout(() => {
        console.log('🔄 [FreeHorizontal] Reinitializing after resize');
    ...
    return () => {
      ...
      if (contextRef.current) {
        console.log('🧹 [FreeHorizontal] GSAP context cleanup - complete revert');
        contextRef.current.revert(); // 모든 GSAP 효과와 DOM 조작 원상복구
        contextRef.current = null;
      }
    };
  }, [children, scrubValue, indicators, preventOverlap, transitionDuration, enableMotionBlur, motionBlurIntensity, findNearestScrollContainer, resizeKey]);
